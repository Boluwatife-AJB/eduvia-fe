...
import type { Meta, Teacher } from "@/types";
...
interface TeachersTableProps<TValue> {
  columns: ColumnDef<Teacher, TValue>[];
  data: Teacher[];
  meta: Meta;
  itemsPerPage: number;
  isLoading?: boolean;
...
export default function TeachersTable<TValue>({
  columns,
  data,
  meta,
  itemsPerPage,
          ...
          {isLoading ? (
            ...
            table.getRowModel().rows.map((row) => (
              <TableRow
                ...
                onClick={
                  () =>
                    router.push(`/${tenant}/admin/teachers/${row.original.id}`)
      ...
      <div className="flex w-full flex-wrap items-center justify-between gap-6 max-sm:justify-center px-6">
        ...
        <div className="text-muted-foreground flex grow items-center justify-end whitespace-nowrap max-sm:justify-center">
          ...
          >
            ...
            <span className="text-foreground">{meta.total}</span> teachers
          </p>
        </div>
